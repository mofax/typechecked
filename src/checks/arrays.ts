...
type ReturnTypeExtractor<T extends Readonly<ValidatorFunction<unknown>[]>, S extends ReturnType<T[number]>[] = []> =
    T["length"] extends S["length"] ? S : ReturnTypeExtractor<T, [...S, ReturnType<T[S['length']]>]>
...
export function isTuple<A extends Readonly<ValidatorFunction<unknown>[]>>(...arg: A) {
    ...
    return function check(value: unknown) {
        ...
        if (arg.length !== value.length) {
            throw new TypeCheckedError(`Expected value to have length ${arg.length} tuple but found ${value.length}`);
        }
